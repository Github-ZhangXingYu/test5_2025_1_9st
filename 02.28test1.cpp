...
QPoint P1, P2;//两个临时点
QPoint P[51];//点数组，防越界
QString Pl[51], pic;//每个点的标签，pic为当前背景图片的存储路径
int ways;//右侧显示栏路径的数量
    ...
    if (event->button() == Qt::LeftButton)
    {
        ...
        switch (tp)
        {
            ...
            case 2://选择第二个点
                ...
                update(); break;
            case 3://添加点
                if (num2 < 50 && temp.x() >= 190 && temp.x() <= 1190 && temp.y()>70 && temp.y() <= 770)//判断所加的点是否在窗口范围内
                {
                    int t = num2++;
                    for (int i = 1; i < num2; i++)
            ...
            case 9://选择终点
                for (int i = 1; i <= num2; i++)
                    if (P[i] != P1 && isin(temp, P[i]))//判断不与起点重合
                    {
                        ...
                        if (ways)//若有路
                        {
                            if (ways > 200000) ui->listWidget->addItem("路径数过多，超过200000条，无法完全显示！最短路径为：" + temp2.getstr() + "，长度为：" + QString::number(min));
                            else ui->listWidget->addItem("共" + QString::number(ways) + "条路径，其中最短路径为：" + temp2.getstr() + "，长度为：" + QString::number(min));
